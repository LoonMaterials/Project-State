...
function main() {
  ...
  assert(!desktopAdapterText.includes("browserFallback"), "Desktop adapter still creates a browser fallback.");
  assert(!desktopAdapterText.includes("browserFileMetadata") && !desktopAdapterText.includes("browserReadFileAsText") && !desktopAdapterText.includes("browserSaveTextFile"), "Desktop adapter still falls back to browser file/download helpers.");
  const browserAdapterText = appText.slice(browserAdapterStart, appText.indexOf("function browserLocalFilePath", browserAdapterStart));
  assert(browserAdapterText.includes('id: "desktop-required"'), "Missing desktop-required adapter identity.");
  assert(/supported\(\) \{\s+return false;\s+\}/.test(browserAdapterText), "Browser adapter can still become a supported runtime.");
  assert(/supportsIndexedDb\(\) \{\s+return false;\s+\}/.test(browserAdapterText), "Browser IndexedDB support is still enabled.");
  ...
  assert(appText.includes("Desktop storage spine is required for Project State saves."), "Project State still appears to allow non-desktop saves.");
  ...
  const requiredDocPieces = [
    ...
    "Old browser JSON can still be imported through the desktop bridge",
