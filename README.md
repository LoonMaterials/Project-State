...
AI Work Orders also support a model-neutral **Universal Review Pack** exchange. Project State exports every stored chunk, cryptographic package identity, and a canonical project registry with stable IDs, aliases, former names, hierarchy, status, and summaries. Private/excluded projects stay out by default. Any local model, cloud model, or human reviewer can return strict schema-valid JSON. The single **Import Reviewed Evidence** action automatically locates the source Work Order/Discovery Case from the returned package identity. Imported results and human review actions are immutable Discovery history only: they cannot change local evidence, Intake, Candidate Map results, or Core. A human must deliberately route an approved proposal through Intake/Airlock. See `UNIVERSAL_AI_REVIEW_EXCHANGE.md`.
...
The first maintained connectors are provider-neutral: `scripts/api-arm-submit.js` submits proposal envelopes, `scripts/api-arm-submit-file.js` submits checksum-verified files under `FILE_ARM_CONTRACT.md`, and `scripts/api-arm-submit-folder.js` submits recursive folders as Discovery-aware file groups. They read the token from `PROJECT_STATE_API_ARM_TOKEN` and refuse token command-line arguments. Folder submission preserves relative paths, creates grouped Discovery Cases, extracts readable files, and runs deterministic document-unit detection on the final file in each group. No provider-specific calendar, email, chat, or Codex connector is installed yet. Local AI analysis can optionally use Qwen3 8B through Ollama as a pre-Airlock Idea Candidate provider; cloud/API AI remains unconfigured.
